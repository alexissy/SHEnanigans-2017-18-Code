...
// This code ALSO requires that you have a Modern Robotics I2C gyro with the name "gyro" * otherwise you would use: PushbotAutoDriveByEncoder; * *
// This code requires that the drive Motors have been configured such that a positive * power command moves them forward, and causes the encoders to count UP. * *
// This code uses the RUN_TO_POSITION mode to enable the Motor controllers to generate the run profile * *
// In order to calibrate the Gyro correctly, the robot must remain stationary during calibration. *
// This is performed when the INIT button is pressed on the Driver Station. *
// This code assumes that the robot is stationary when the INIT button is pressed. *
// If this is not the case, then the INIT should be performed again. * *
// Note: in this example, all angles are referenced to the initial coordinate frame set during the * the Gyro Calibration process, or whenever the program issues a resetZAxisIntegrator() call on the Gyro. * *
// The angle of movement/rotation is assumed to be a standardized rotation around the robot Z axis, * which means that a Positive rotation is Counter Clock Wise, looking down on the field. *
...
@Autonomous(name="Red Autonomous: CryptoKeyVuMarks", group="Red_Auto:")
public class RedAutonomous_CryptoKeyVuMarks extends LinearOpMode {
    ...
    public void runOpMode() {
        /* * Initialize the standard drive system variables. * The init() method of the hardware class does most of the work here */
        ...
        parameters.accelerationIntegrationAlgorithm = new JustLoggingAccelerationIntegrator(); // Retrieve and initialize the IMU. We expect the IMU to be attached to an I2C port on a Core Device Interface Module, configured to be a sensor of type "AdaFruit IMU", // and named "imu".

        imu = hardwareMap.get(BNO055IMU.class, "imu");
        imu.initialize(parameters); // Ensure the robot it stationary, then reset the encoders and calibrate the gyro.
     ...
     *  Should allow for _/- variance for adjusting heading *
    ...
    public void gyroDrive ( double speed, double distance, double angle) {
        ...
        if (opModeIsActive()) {
            ...
            newFrontLeftTarget = robot.leftDrive.getCurrentPosition() + moveCounts;
            newFrontRightTarget = robot.rightDrive.getCurrentPosition() + moveCounts;
            //newBackLeftTarget = robot.pulleyMotor.getCurrentPosition() + moveCounts;
            //newBackRightTarget = robot.motorBackRight.getCurrentPosition() + moveCounts;

            // Set Target and Turn On RUN_TO_POSITION
            robot.leftDrive.setTargetPosition(newFrontLeftTarget);
            robot.rightDrive.setTargetPosition(newFrontRightTarget);
     ...
     * Centered on the robot's frame of reference * +ve error means the robot should turn LEFT (CCW) to reduce error. */
